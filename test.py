...
def performance_test(num_of_test = 10, n = 5):
    ...
    for _ in num_of_test:
        ...
        total_distance += calculate_metrics_score(shortest_distance, env.num_steps)
    return total_distance/num_of_test

def main():
    from agent import Agent, Trainer
    environment = Environment(n=5, with_animation=False) 
    agent = Agent()
